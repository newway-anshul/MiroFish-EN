<template>
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG short-term memory real-time update' : 'Real-time update...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still being processed, refresh the graph manually later if needed</span>
    ...
    <!-- Bottom Legend Area (Bottom Left) -->
...
// Watch isSimulating change to detect when simulation ends
...
const renderGraph = () => {
  ...
  // Process edge data, calculate number and index of edges between same node pairs
  const edgePairCount = {}
  ...
  // Record which edge we're processing for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop edge - add only one merged self-loop for each node
    ...
    // Calculate curvature: spread multiple edges, single edge straight
    let curvature = 0
    if (totalCount > 1) {
      ...
      // If edge direction reverses normalized direction, flip curvature
      // This ensures all edges are distributed in the same reference, no overlap due to different directions
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add attractive force towards center to cluster isolated node groups
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from node's right side, circle back
    ...
    // Offset perpendicular to the line direction, calculated based on distance ratio, ensure curve is clearly visible
    // More edges, the larger the offset ratio to distance
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop label position: node's right side
      return { x: sx + 70, y: sy }
    }
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if actually started dragging (moved beyond threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Only restart simulation when actually dragging is first detected
          d._isDragging = true
