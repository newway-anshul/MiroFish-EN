            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generated, but the whole section may not be complete yet) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll position to keep the button in place
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract sub-questions completely with no count limit
    ...
    // Extract key facts completely with no count limit
    ...
    // Extract core entities completely, including summary and related fact count
    ...
    // Extract relation chains completely with no count limit
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract currently valid facts completely with no count limit
    ...
    // Extract historical/expired facts completely with no count limit
    ...
    // Extract related entities completely with no count limit
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract rationale for interviewee selection
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: 1. **alumni_345 (index=1)**: as a WHU alumnus...
        ...
        // Example: - choose parent_601 (index 0): as a representative parent...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // Continuation lines of rationale (excluding ending summary blocks)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        // Split questions by numeric indexes
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          // If only Reddit answer exists, copy it as default display when it is not placeholder text
          ...
          // No platform markers (very old format), treat as a single full answer
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts completely with no count limit
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sub-questions generated from drift query analysis'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Get current platform selection for a specific question
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Set platform selection for a specific question
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No response received on this platform)' || t === '(No response received on this platform)' || t === '[No response]'
    }

    // Try splitting answers by question index
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "QuestionX:" format
      // 2. "1. " or "\n1. " (number+dot, backward compatible)
      let matches = []
      let match

      // First try the "QuestionX:" format
      const cnPattern = /(?:^|[\r\n]+)Question(\d+):\s*/g
      ...
      // If no match, fallback to the "number." format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // If no numbering is found or only one is found, return the whole text
      if (matches.length <= 1) {
    ...
    // Get answer corresponding to a specific question
    ...
    // Check whether a question has dual-platform answers (excluding placeholders)
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch buttons (shown only when both platforms have real answers)
...
const activeStep = computed(() => {
  ...
  // If no active step exists, return the last done step
...
const renderMarkdown = (content) => {
  ...
  // Fix numbering of non-consecutive ordered lists: keep incrementing when single-item <ol> blocks are separated by paragraphs
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Handle scroll logic uniformly in nextTick after loop completion
          }
...
const extractFinalContent = (response) => {
  ...
  // Try extracting content after "Final Answer:" (supports multiple formats)
  ...
  // Additional fallback for repeated "Final Answer" marker forms
  ...
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove thought process prefixed by "Thought:"
