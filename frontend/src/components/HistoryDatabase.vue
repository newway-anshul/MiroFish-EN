        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be started during runtime, history playback not supported</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Expanded state: dynamically calculate height based on number of cards
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate the number of cards in current row, ensure each row is centered
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the target state to be executed (record the latest target state regardless of whether animating)
        ...
        // If target state is same as current state, no need to process
        ...
        // Shorter delay for expansion (50ms), longer delay for collapse (200ms) for increased stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // After animation completes, release lock and check if there are pending state changes
          setTimeout(() => {
            isAnimating = false
            
            // After animation ends, check if there is a new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay a short time before execution to avoid switching too fast
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, shrink viewport bottom upwards, need to scroll more to trigger expansion
...
/* Use CSS background patterns to create a square grid with fixed spacing */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Anchor at top-left; when height changes, only expand downward without shifting existing grid */
...
.cards-container {
  ...
  /* min-height is dynamically calculated by JS and adapts to card count */
}
...
/* Low-saturation color palette - Morandi style */
