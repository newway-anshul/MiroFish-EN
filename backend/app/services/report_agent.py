...
Uses LangChain + Zep to generate simulation reports in ReACT mode.
...
class ReportLogger:
    ...
    Each line is a full JSON object containing timestamp, action type, and details.
    """
    ...
    def log(
        ...
        Args:
            action: Action type, such as 'start', 'tool_call', 'llm_response', 'section_complete'.
    ...
    def log_planning_context(self, context: Dict[str, Any]):
        """Log context gathered during planning."""
        self.log(
    ...
    def log_section_content(
        ...
        """Log section content generation completion (content only)."""
        self.log(
    ...
    def log_section_full_complete(
        ...
        Frontend should watch this log entry to determine true section completion
        and fetch the complete content.
        """
        self.log(
...
class ReportConsoleLogger:
    ...
    Writes console logs (INFO, WARNING, etc.) to `console_log.txt` in the
    report folder. Unlike `agent_log.jsonl`, this is plain-text output.
    """
    ...
    def __init__(self, report_id: str):
        ...
        Args:
            report_id: Report ID, used to determine log path.
        """
        self.report_id = report_id
        self.log_file_path = os.path.join(
    ...
    def _setup_file_handler(self):
        """Set up file handler and mirror logs to file."""
        import logging
        ...
        # Use the same compact format as console logs.
    ...
    def close(self):
        """Close and detach the file handler from loggers."""
        import logging
...
class ReportAgent:
        ...
        return {
            "insight_forge": {
                ...
                "parameters": {
                    ...
                    "report_context": "Current section context (optional, helps generate sharper sub-questions)"
                }
            },
            "panorama_search": {
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # Deep interview - call real OASIS interview APIs (dual platform)
            ...
            # ========== Backward-compatible legacy tools (internally redirected) ==========
                ...
                # Redirect to insight_forge because it is stronger
    ...
    # Valid tool names used to validate bare-JSON fallback parsing
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Format 2: Fallback - LLM outputs bare JSON (without <tool_call> tag)
        # Only try this when format 1 does not match, to avoid false positives.
        ...
        # Response may include thought text + bare JSON; extract the last JSON object.
    ...
    def plan_outline(
        ...
        Analyze simulation requirements using LLM and generate report structure.
        ...
        try:
            ...
            # Return default 3-section fallback outline
    ...
    def _generate_section_react(
        ...
        """
        Generate one section using ReACT mode.

        ReACT loop:
        1. Thought - analyze what information is needed.
        2. Action - call tools to fetch information.
        ...
        # Build user prompt - pass up to 4000 chars per completed section
        ...
        conflict_retries = 0  # Consecutive conflicts where tool call and Final Answer both appear
        ...
        for iteration in range(max_iterations):
            ...
            # Check if LLM response is None (API issue or empty content)
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # If last iteration is also None, break and force finalize
                break
                ...
                if conflict_retries <= 2:
                    ...
                    messages.append({
                        ...
                        "content": (
                            ...
                            "Each reply can do only one of the following:\n"
                            ...
                            "Please reply again and do only one option."
                        ),
                    })
                    continue
                else:
                    # Third time: degrade by truncating to first tool call and force execution
                    logger.warning(
            ...
            # -- Case 1: LLM produced Final Answer --
            if has_final_answer:
                # Not enough tool calls: reject and require more tool usage
                if tool_calls_count < min_tool_calls:
                    messages.append({"role": "assistant", "content": response})
                    unused_tools = all_tools - used_tools
                    unused_hint = f"(Unused tools: {', '.join(unused_tools)}. Consider trying them.)" if unused_tools else ""
                    messages.append({
                        "role": "user",
            ...
            if has_tool_calls:
                # Tool quota exhausted -> ask for Final Answer
            ...
            if tool_calls_count < min_tool_calls:
                # Tool calls still insufficient; suggest unused tools
                unused_tools = all_tools - used_tools
                unused_hint = f"(Unused tools: {', '.join(unused_tools)}. Consider trying them.)" if unused_tools else ""
            ...
            # Tool calls are sufficient; if output lacks "Final Answer:" prefix,
            # accept response as final content to avoid extra loops.
            logger.info(f"Section {section.title} has no 'Final Answer:' prefix; accept LLM output as final content (tool calls: {tool_calls_count})")
        ...
        # Max iterations reached, force final generation
        ...
        # Check if forced-finalization response is None
        if response is None:
            logger.error(f"Section {section.title}: LLM returned None during forced finalization; using default error text")
    ...
    def generate_report(
        ...
        """
        Generate a complete report (real-time section output).

        Each generated section is saved immediately; no need to wait for whole report.
        File structure:
        reports/{report_id}/
        ...
        try:
            ...
            # Stage 2: generate per section and save incrementally
            ...
            # Assemble full report via ReportManager
    ...
    def chat(
        ...
        During chat, the agent may autonomously call retrieval tools.
        ...
        try:
            ...
            if report and report.markdown_content:
                # Limit report length to avoid oversized context
        ...
        for iteration in range(max_iterations):
            ...
            tool_results = []
            for call in tool_calls[:1]:  # Execute at most 1 call per round
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        This is plain console output during report generation (INFO/WARNING/etc.),
        different from structured `agent_log.jsonl` logs.
        
        Args:
            report_id: Report ID
            from_line: Start reading from this line for incremental fetch (0 means from beginning)
        ...
        with open(log_path, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                total_lines = i + 1
                if i >= from_line:
                    # Keep original line and strip trailing newline
    ...
    def save_section(
        ...
        Called right after each section is generated, enabling incremental output.
        ...
        # Build section Markdown and clean potentially duplicated headings
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        2. Convert all level-3-and-deeper headings to bold text.
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Convert headings (#, ##, ###, ####, etc.) to bold text.
                # Section title is system-added, so body should not contain headings.
                cleaned_lines.append(f"**{title_text}**")
                cleaned_lines.append("")  # Add blank line
                continue
            
            # If previous line was a skipped heading and current is empty, skip it too
    ...
    def update_progress(
        ...
        Frontend can poll `progress.json` for real-time progress.
        """
        cls._ensure_report_folder(report_id)
    ...
    def get_generated_sections(cls, report_id: str) -> List[Dict[str, Any]]:
        ...
        Returns all saved section file information.
        """
        folder = cls._get_report_folder(report_id)
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        Build full report from saved section files and clean heading issues.
        """
        folder = cls._get_report_folder(report_id)
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                if is_duplicate:
                    # Skip duplicated heading and trailing empty lines
                    i += 1
                ...
                # - # (level=1): keep report main heading only
                ...
                if level == 1:
                    if title == outline.title:
                        ...
                        # Section heading mistakenly used #; normalize to ##
                    ...
                    # Convert level-3-and-deeper headings to bold text
                    processed_lines.append(f"**{title}**")
                    processed_lines.append("")
                    prev_was_heading = False
                ...
                # Skip separator directly after a heading
                i += 1
                continue
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        if not os.path.exists(path):
            # Backward compatibility: check legacy files stored directly under reports/
    ...
    def delete_report(cls, report_id: str) -> bool:
        ...
        # Legacy format: delete standalone files
        deleted = False
