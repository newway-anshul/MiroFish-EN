...
Runs simulations in the background and records each agent action, with real-time status monitoring support.
"""
...
# Flag indicating whether cleanup handler has been registered
...
class SimulationRunState:
    """Simulation runtime state (real-time)."""
    simulation_id: str
    ...
    # Per-platform round indices and simulated time (for dual-platform parallel display)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Per-platform round indices and simulated time
...
class SimulationRunner:
    ...
    2. Parse runtime logs and record each agent action
    3. Provide real-time state query APIs
    4. Support pause/stop/resume operations
    """
    
    # Runtime state storage directory
    ...
    def _load_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        ...
        try:
            ...
            logger.error(f"Failed to load runtime state: {str(e)}")
            return None
    ...
    def start_simulation(
        ...
        enable_graph_memory_update: bool = False,  # Whether to push activities to Zep graph
        ...
        if not os.path.exists(config_path):
            raise ValueError("Simulation config does not exist. Please call the /prepare API first")
        ...
        try:
            ...
            # Set subprocess environment variables to enforce UTF-8 on Windows
            # This fixes third-party libraries (e.g., OASIS) that read files without specifying encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Supported in Python 3.7+, makes open() default to UTF-8
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensure stdout/stderr use UTF-8
            
            # Set working directory to simulation directory (database and related files are generated here)
            # Use start_new_session=True to create a new process group, enabling termination of all child processes via os.killpg
            process = subprocess.Popen(
                cmd,
                cwd=sim_dir,
                stdout=main_log_file,
                stderr=subprocess.STDOUT,  # Write stderr to the same file as stdout
                text=True,
            ...
            cls._stderr_files[simulation_id] = None  # Separate stderr handle is no longer needed
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # Read logs one last time after process exits
            if os.path.exists(twitter_actions_log):
            ...
            if exit_code == 0:
                ...
                state.runner_status = RunnerStatus.FAILED
                # Read error details from main log file
                main_log_path = os.path.join(sim_dir, "simulation.log")
                error_info = ""
                try:
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # Check whether all enabled platforms are completed
                                    # If only one platform is running, check that one
                                ...
                                # Update round information (from round_end event)
                                    ...
                                    # Update per-platform rounds and simulated time
                                    if platform == "twitter":
                                    ...
                                    # Global round uses the maximum across platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Global simulated time uses maximum across platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Platform enablement is inferred by checking whether corresponding `actions.jsonl` files exist.
        ...
        # Return False if an enabled platform is not completed
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Terminate process and its child processes across platforms.
        ...
        if IS_WINDOWS:
            ...
            try:
                ...
                # Force SIGKILL if still alive after timeout
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Skip non-action records (e.g., simulation_start, round_start, round_end)
                    ...
                    # Resolve platform: prefer record platform, otherwise use default platform
    ...
    def get_all_actions(
        ...
        # Read Twitter action log (auto-set platform to twitter based on file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read Reddit action log (auto-set platform to reddit based on file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        ...
        # If per-platform files do not exist, try legacy single-file format
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Legacy format should include platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean simulation runtime logs (used for forced restart).
        ...
        Note: Configuration file (`simulation_config.json`) and profile files are not deleted.
        ...
        if not os.path.exists(sim_dir):
            return {"success": True, "message": "Simulation directory does not exist; no cleanup required"}
        ...
        # Directories to clean (contains action logs)
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check whether there is anything to clean (avoid noisy logs when nothing is running)
    ...
    def register_cleanup(cls):
        ...
        Called when Flask app starts to ensure all simulation processes are cleaned up on shutdown.
        """
        ...
        # In Flask debug mode, register cleanup only in the reloader child process (the actual app process)
        # WERKZEUG_RUN_MAIN=true means reloader child process
        # In non-debug mode this env var is absent, so registration is still needed
        ...
        # Register atexit handler (fallback)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None: interview both platforms in dual-platform mode and return merged result
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                - "twitter": default to Twitter-only interviews
                - "reddit": default to Reddit-only interviews
    ...
    def interview_all_agents(
        ...
        Use the same prompt to interview all Agents in the simulation.
        ...
        # Read all Agent info from config file
        config_path = os.path.join(sim_dir, "simulation_config.json")
        if not os.path.exists(config_path):
    ...
    def close_simulation_env(
        ...
        Sends a close-environment command so the simulation exits command-wait mode gracefully.
        ...
        try:
            ...
            # Timeout may happen because environment is currently closing
            return {
                "success": True,
    ...
    def get_interview_history(
        ...
        Args:
            ...
            platform: Platform type (reddit/twitter/None)
                - "reddit": only retrieve Reddit history
                - "twitter": only retrieve Twitter history
                - None: retrieve history from both platforms
            agent_id: Specific Agent ID (optional, only retrieve this Agent's history)
        ...
        # Determine target platforms to query
        ...
        # If multiple platforms were queried, cap total result size
